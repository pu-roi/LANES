...
- **Always Keep Dependency and Migration State Synchronized**: Whenever adding or importing new Python libraries or Node packages, you MUST immediately update `backend/requirements.txt` or `frontend/package.json`. Before pushing or concluding tasks involving model/schema updates, verify that Alembic migrations (`alembic upgrade head`) are tested and cleanly documented so that pulling the repository on other computers works out-of-the-box without missing dependencies or broken schemas.
...
2. **Pre-Push Sync Check:** Verify that any newly imported Python/Node packages are recorded in `backend/requirements.txt` or `frontend/package.json`, and ensure any new database migrations run cleanly (`alembic upgrade head`) so that clones on other machines do not fail.
3. **Stage all changes:** `git add -A`
4. **Synthesize the commit message:** Review the conversation and create a message following Conventional Commits (e.g., `feat(map): added heatmap layer`). Write a bulleted body explaining the *what* and *why*.
5. **Commit:** `git commit -m "<summary line>" -m "<bulleted body>"`
