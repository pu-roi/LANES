...
The collector must not create a public flood report, activate a zone, change routing, or declare a flood verified. Those decisions belong to later extraction, location resolution, and staff moderation work. The Pasig map border is visual; an article about another Philippine city must remain eligible for the same review workflow.
...
4. **Filter before opening articles.** Reject clearly irrelevant or stale entries; shortlist flood terms with Philippine province, city/municipality, barangay, road, or landmark context. A title may omit the place while the excerpt or accessible article body identifies it. Keep uncertain Philippine flood leads for later extraction or staff inspection instead of discarding them because “Pasig” is absent. A publisher's Philippine identity alone does not prove that an article describes flooding in the Philippines.
...
- Flood articles from Pasig and at least two other Philippine regions are shortlisted and their accessible text/evidence retained; an irrelevant item does not trigger full retrieval. A blocked article stays a metadata-only lead and is never passed as full-text evidence. Tests include a Philippine place absent from the headline and a non-Philippine flood story from an approved publisher.
