...
> - [x] **Pending-Report Aura Endpoint Alignment (`usePendingReportsLayer.ts`)** ([@roicambe](https://github.com/roicambe) (Roi Cambe)): Preserved the existing transparent severity aura, zoom behavior, selected emphasis, and rounded turns while ending it exactly at the persisted validated road geometry so wide round-cap bulbs no longer spill into adjacent properties. The shared layer also resynchronizes its cap after panel navigation or Fast Refresh instead of retaining a stale MapLibre layout.
