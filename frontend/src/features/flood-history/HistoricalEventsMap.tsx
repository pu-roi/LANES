...
export function HistoricalEventsMap({ events, selectedEventId, onSelectEvent }: HistoricalEventsMapProps) {
  ...
  return <div className="relative h-[22rem] overflow-hidden rounded-xl border border-slate-200 bg-slate-100 lg:h-full lg:min-h-[38rem]"><BaseMap onMapLoad={handleMapLoad} center={[121.085, 14.576]} zoom={12.7} className="h-full w-full" /><div className="pointer-events-none absolute left-3 top-3 rounded-lg bg-white/95 px-3 py-2 text-xs text-slate-600 shadow-sm"><p className="font-semibold text-slate-800">Verified event footprints</p><p>Historical only — never used for live routing.</p></div><div className="pointer-events-none absolute bottom-3 left-3 flex gap-2 rounded-lg bg-white/95 p-2 text-[11px] text-slate-600 shadow-sm"><span><i className="mr-1 inline-block h-2 w-2 rounded-full bg-lime-500" />Low</span><span><i className="mr-1 inline-block h-2 w-2 rounded-full bg-amber-500" />Medium</span><span><i className="mr-1 inline-block h-2 w-2 rounded-full bg-orange-500" />High</span><span><i className="mr-1 inline-block h-2 w-2 rounded-full bg-red-600" />Extreme</span></div></div>;
}
