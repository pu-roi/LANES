...
export function FloodEventDetailModal({ eventId, onClose, onFocusHistoricalMap }: { eventId: number | null; onClose: () => void; onFocusHistoricalMap: (target: HistoricalMapFocusTarget) => void }) {
  ...
  return <><AnimatePresence>{eventId !== null && <motion.div aria-labelledby="flood-event-details-title" aria-modal="true" className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-3 backdrop-blur-xs sm:p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={handleClose} role="dialog">
    <motion.div className="relative flex max-h-[calc(100dvh-1.5rem)] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl sm:max-h-[90vh]" initial={{ opacity: 0, scale: 0.97, y: 12 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.97, y: 12 }} transition={{ duration: 0.18 }} onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className={`min-h-0 flex-1 p-5 pb-[calc(var(--bottom-nav-height)+env(safe-area-inset-bottom))] sm:p-6 sm:pb-[calc(1.5rem+env(safe-area-inset-bottom))] ${activeTab === "reports" ? "overflow-y-auto lg:flex lg:flex-col lg:overflow-hidden" : "overflow-y-auto"}`}>
        ...
        {data && selectedReport ? <section className="space-y-5">
          ...
          <div className="grid gap-3 sm:grid-cols-2"><div className="rounded-xl border border-slate-200 p-4"><p className="text-xs text-slate-500">Flood survey</p><p className="mt-1 text-sm text-slate-800">Depth: {formatFloodDepth(selectedReport.depth) || "Not specified"}</p><p className="text-sm text-slate-800">Coverage: {selectedReport.is_bidirectional ? "Both directions" : "One direction or not recorded"}</p><p className="text-sm text-slate-800">Passability: {selectedReport.survey?.passable_vehicles || selectedReport.passable_vehicles || "Not recorded"}</p><p className="text-sm text-slate-800">Hidden hazards: {selectedReport.survey?.hidden_hazards || selectedReport.hidden_hazards || "Not recorded"}</p></div><details className="rounded-xl border border-slate-200 p-4"><summary className="cursor-pointer text-xs text-slate-500">Exact original geometry</summary><pre className="mt-2 max-h-32 overflow-auto whitespace-pre-wrap break-all text-xs text-slate-700">{selectedReport.geometry ? JSON.stringify(selectedReport.geometry) : "No geometry recorded"}</pre></details></div>
          ...
          <section className="grid gap-3 sm:grid-cols-2"><div><h3 className="font-semibold text-slate-900">Incident summary</h3><p className="mt-2 text-sm text-slate-700">Peak severity: <span className="font-semibold capitalize">{data.peak_severity}</span>{data.peak_depth ? ` · ${formatFloodDepth(data.peak_depth)}` : ""}</p><p className="mt-1 text-sm text-slate-600">{data.supporting_report_count ?? data.reports.length} linked report{(data.supporting_report_count ?? data.reports.length) === 1 ? "" : "s"} from {data.reporter_count ?? 0} identified reporter{(data.reporter_count ?? 0) === 1 ? "" : "s"} · {data.media_count ?? 0} evidence file{(data.media_count ?? 0) === 1 ? "" : "s"}.</p></div><div><h3 className="font-semibold text-slate-900">Official zone history</h3><p className="mt-2 text-sm text-slate-600">Zone records document the official footprint; they are not live-routing advisories.</p><div className="mt-2 space-y-2">{data.zones.map((zone) => <div key={zone.id} className="flex items-center justify-between gap-3 rounded-lg bg-slate-50 px-3 py-2 text-xs text-slate-700"><span>Zone #{zone.id} · <span className="capitalize">{zone.severity}</span>{zone.depth ? ` · ${formatFloodDepth(zone.depth)}` : ""}<span className="block text-slate-500">Created {formatDate(zone.created_at)}</span></span><span className={zone.is_active ? "font-semibold text-emerald-700" : "text-slate-500"}>{zone.is_active ? "Active" : "Ended"}</span></div>)}</div></div></section>
