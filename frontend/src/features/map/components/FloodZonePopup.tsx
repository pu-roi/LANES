import React from "react";
import { format } from "date-fns";
import { Clock, Ruler, Car, EyeOff, ShieldCheck, User } from "lucide-react";

interface FloodZonePopupProps {
  properties: any;
}

const SEVERITY_COLORS: Record<string, string> = {
  low: "#84cc16",
  medium: "#eab308",
  high: "#f97316",
  extreme: "#ef4444",
};

export const FloodZonePopup: React.FC<FloodZonePopupProps> = ({ properties }) => {
  const {
    severity = "medium",
    color = SEVERITY_COLORS["medium"],
    created_at,
    expires_at,
    report_text,
    reporter_name,
    reporter_role,
    depth,
    passable_vehicles,
    hidden_hazards,
  } = properties;

  let reportedText = "Unknown";
  if (created_at) {
    try {
      reportedText = format(new Date(created_at), "MMM d, h:mm a");
    } catch (e) {}
  }

  // Determine badge styling based on role
  let badgeColor = "bg-gray-100 text-gray-700 border-gray-200";
  if (reporter_role === "Admin" || reporter_role === "Super Admin") {
    badgeColor = "bg-red-100 text-red-700 border-red-200";
  } else if (reporter_role === "Moderator") {
    badgeColor = "bg-purple-100 text-purple-700 border-purple-200";
  } else if (reporter_role === "DRRM Officer") {
    badgeColor = "bg-blue-100 text-blue-700 border-blue-200";
  } else if (reporter_role === "Commuter") {
    badgeColor = "bg-green-100 text-green-700 border-green-200";
  }

  return (
    <div className="flex flex-col min-w-[280px] max-w-[320px] font-sans bg-white shadow-sm rounded-xl overflow-hidden">
      {/* Header */}
      <div 
        className="flex items-center justify-between px-4 py-3"
        style={{ backgroundColor: color, color: "#ffffff" }}
      >
        <div className="flex items-center gap-2">
          <div
            className="w-2.5 h-2.5 rounded-full bg-white shadow-sm animate-pulse"
          />
          <h3 className="text-base font-bold capitalize tracking-tight">
            {severity} Risk
          </h3>
        </div>
        <div className="flex items-center text-xs font-medium bg-white/20 px-2 py-1 rounded-md backdrop-blur-sm">
          <Clock className="w-3 h-3 mr-1 opacity-80" />
          {reportedText}
        </div>
      </div>

      <div className="p-4">
        {/* Description */}
        {report_text && report_text !== "null" && (
          <div className="text-sm text-gray-700 mb-5 leading-relaxed bg-slate-50 p-3 rounded-lg border border-slate-100 italic">
            "{report_text}"
          </div>
        )}

        {/* Details Grid */}
        <div className="grid grid-cols-2 gap-y-4 gap-x-3 mb-2">
          {/* Depth */}
          <div className="flex flex-col gap-1">
            <div className="flex items-center text-[10px] font-bold text-gray-400 uppercase tracking-wider">
              <Ruler className="w-3 h-3 mr-1" />
              Height
            </div>
            <div className="text-sm font-semibold text-gray-900">
              {depth && depth !== "null" ? depth.replace(/_/g, ' ') : "Not specified"}
            </div>
          </div>

          {/* Hidden Hazards */}
          <div className="flex flex-col gap-1">
            <div className="flex items-center text-[10px] font-bold text-gray-400 uppercase tracking-wider">
              <EyeOff className="w-3 h-3 mr-1" />
              Hazards
            </div>
            <div className="text-sm font-semibold text-gray-900 capitalize">
              {hidden_hazards && hidden_hazards !== "null" ? hidden_hazards : "Unsure"}
            </div>
          </div>

          {/* Passable Vehicles */}
          <div className="col-span-2 flex flex-col gap-1 mt-1">
            <div className="flex items-center text-[10px] font-bold text-gray-400 uppercase tracking-wider">
              <Car className="w-3 h-3 mr-1" />
              Safe to pass for
            </div>
            <div className="text-sm font-semibold text-gray-900">
              {passable_vehicles && passable_vehicles !== "null" ? passable_vehicles.replace(/_/g, ' ') : "Not specified"}
            </div>
          </div>
        </div>
      </div>

      {/* Footer (Reporter) */}
      <div className="flex items-center justify-between border-t border-gray-100 bg-gray-50/80 px-4 py-3">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 bg-blue-100 rounded-full flex items-center justify-center text-blue-600">
            {reporter_role?.includes("Admin") || reporter_role?.includes("DRRM") ? (
              <ShieldCheck className="w-4 h-4" />
            ) : (
              <User className="w-4 h-4" />
            )}
          </div>
          <div className="flex flex-col">
            <span className="text-xs font-bold text-gray-900 leading-tight">
              {reporter_name && reporter_name !== "null" ? reporter_name : "System"}
            </span>
            <span className="text-[10px] text-gray-500 font-medium leading-tight">
              Reporter
            </span>
          </div>
        </div>
        {reporter_role && reporter_role !== "null" && (
          <div className={`px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider border rounded-full ${badgeColor}`}>
            {reporter_role}
          </div>
        )}
      </div>
    </div>
  );
};
