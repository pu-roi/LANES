...
async def send_password_change_otp_email_async(to_email: str, otp_code: str) -> tuple[bool, str]:
    """
    Sends a Password Change OTP email using Resend REST API with clean hosted branding.
    Returns (success, error_message)
    """
    api_key = settings.effective_resend_api_key
    if not api_key:
        print(f"WARN: Resend API Key not set. Simulating Password Change OTP {otp_code} to {to_email}")
        return True, ""
        ...
        <table align="center" border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width: 540px; background-color: #ffffff; border-radius: 16px; box-shadow: 0 4px 24px rgba(15, 23, 42, 0.06); border: 1px solid #e2e8f0; overflow: hidden; margin: 0 auto;">
            ...
            <tr>
                <td style="padding: 36px 36px 32px 36px;">
                    ...
                    <p style="color: #64748b; font-size: 15px; line-height: 1.6; margin: 0 0 28px 0; text-align: center;">
                        We received a request to update the password for your <strong>LANES</strong> account. Please enter the verification code below to authorize this change.
                    </p>
                    ...
                    <p style="color: #94a3b8; font-size: 13px; line-height: 1.5; margin: 0 0 28px 0; text-align: center;">
                        This code will expire in <strong style="color: #64748b;">5 minutes</strong>.<br>If you did not make this request, please change your password immediately or contact support.
                    </p>
    ...
    payload: dict = {
        "from": settings.RESEND_FROM_EMAIL,
        "to": [to_email],
        "subject": "LANES Password Change Verification Code",
        "html": html_content
    }
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            try:
                ...
                err_msg = f"Resend HTTP {e.response.status_code}: {e.response.text}"
            print(f"Error sending password change email via Resend API: {err_msg}")
            return False, err_msg
        except Exception as e:
            err_msg = f"Resend Error: {str(e)}"
            print(f"Error sending password change email via Resend API: {err_msg}")
